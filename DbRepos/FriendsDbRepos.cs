...
public class FriendsDbRepos
{
    ...
    {
        return await _dbContext.Friends.CountAsync();
    }
    ...
    public async Task<ResponsePageDto<IFriend>> ReadFriendsAsync(bool seeded, bool flat, string? country, string? city, string? filter, int pageNumber, int pageSize)
    {
        ...
        {
            var c = country!.Trim();
            query = query.Where(i => i.AddressDbM != null && i.AddressDbM.Country == c);
        }

        if (!string.IsNullOrWhiteSpace(city))
        {
            var ci = city!.Trim();
            query = query.Where(i => i.AddressDbM != null && i.AddressDbM.City == ci);
        }

        var totalCount = await query.CountAsync();

        var pageItemsEntities = await query
        ...
        var pageItems = pageItemsEntities.Cast<IFriend>().ToList();
